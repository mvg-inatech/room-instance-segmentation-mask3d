...
class MultiPoly(Dataset):
    def __init__(self, img_folder, ann_file, transforms, semantic_classes, dataset_name: str, valid_scenes_file_path=None):
        ...
        if dataset_name == "stru3d":
            ...
            assert valid_scenes_file_path is not None, "valid_scenes_file_path must be provided for Structured3D dataset"
            with open(valid_scenes_file_path, "r") as f:
                valid_scene_names = f.read().splitlines()
            self.ids = [img_id for img_id in self.ids if f"scene_{img_id:05d}" in valid_scene_names]
...
class ConvertToCocoDict(object):
    ...
    def __call__(self, img_id, path, target):
        ...
        if self.augmentations is None:
            ...
            annos = [transform_instance_annotations(obj, transforms, image.shape[:2]) for obj in record.pop("annotations") if obj.get("iscrowd", 0) == 0]
...
def build(image_set, args):
    ...
    dataset = MultiPoly(
        ...
        valid_scenes_file_path=args.valid_scenes_file_path,
    )

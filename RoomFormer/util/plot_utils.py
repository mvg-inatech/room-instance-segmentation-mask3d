...
def plot_floorplan_with_regions(regions, corners=None, edges=None, scale=256):
    ...
    if len(regions) > 1:
        ...
        # Fix ValueError: setting an array element with a sequence. The requested array has an inhomogeneous shape after 1 dimensions. The detected shape was (7,) + inhomogeneous part.
        avg_corner = [region.mean(axis=0) for region in regions]
        ind = np.argsort([np.square(np.array(avg)).sum() for avg in avg_corner])

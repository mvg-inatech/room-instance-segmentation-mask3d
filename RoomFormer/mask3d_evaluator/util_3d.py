...
import mask3d_evaluator.util as util
...
class Instance(object):
    ...
    def __init__(self, mesh_vert_instances, instance_id, class_label):
        if instance_id == -1:
            return
        
        self.instance_id = int(instance_id)
        self.label_id = class_label
        self.vert_count = int(
...
def get_instances_per_classes(points_instance_id, valid_class_ids, valid_class_names, instance_idx_to_label):
    instances = {}

    for class_name in valid_class_names:
        instances[class_name] = []

    instance_ids_unique = np.unique(points_instance_id)

    for instance_id in instance_ids_unique:
        if instance_id == 0:
            # Point does not belong to an instance
            continue

        instance_idx = instance_id - 1
        class_id = instance_idx_to_label[instance_idx]
        if class_id not in valid_class_ids:
            continue

        instance = Instance(points_instance_id, instance_id, class_name)
        instances[class_name].append(instance.to_dict())
